...
// --- Day/Night Cycle ---
const DayNightCycle = ({ day }: { day: number }) => {
  ...
  useFrame((state) => {
    ...
    if (directionalLightRef.current) {
      ...
      if (isNight) {
        ...
        directionalLightRef.current.intensity = 2;
        directionalLightRef.current.color.setHSL(0.1, 0.2, 1);
      }
    }

    if (ambientLightRef.current) {
      ambientLightRef.current.intensity = sin > 0 ? 0.6 : 0.2;
    }
  });
          ...
          {grid.map((row, y) => row.map((tile, x) => {
            ...
            return (
              <React.Fragment key={`${x}-${y}`}>
                <GroundTile type={tile.buildingType} x={x} y={y} grid={grid} onHover={handleHover} onLeave={handleLeave} onClick={onTileClick} />
                <group position={[wx, 0, wz]} raycast={() => null}>
                  {tile.buildingType !== BuildingType.None && tile.buildingType !== BuildingType.Road && BUILDINGS[tile.buildingType] && (
                    <ProceduralBuilding type={tile.buildingType} baseColor={BUILDINGS[tile.buildingType].color} x={x} y={y} />
                  )}
                </group>
              </React.Fragment>
            )
